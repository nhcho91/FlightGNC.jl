...
function Bias_IACG_StationaryTarget(s_guidance::BPNG, x, t)
    ...
    elseif case == 5
        ...
        Ω_μ  = -k_μ * ṙ * μ   # for desired manoeuvre plane alignment
    else
        Ω_μ  = 0
    end

    if case >= 3
        k_σ  = 2 / pi * acos((σ_M / σ_M_lim)^k)   # for look angle limiting
    else
        k_σ  = 1
    end
    ...
    ω_f      = -k_e * ṙ * e_v̂_f^α * normalize(cross(v̂_f_pred, v̂_f_d)) + Ω_μ * v̂_f_d
